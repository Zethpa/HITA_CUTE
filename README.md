...
### 本分支增加的功能
- 自定义背景图：从手机读入图片，进行裁剪、扩展（适应）、拉伸，并可以调节透明度并选择加入毛玻璃效果  
  入口：功能中心右上角 -> 自定义背景图片
- 更多的主题色：在默认的蓝色基础上增加至了20种可选主题色，并可以根据背景提取出特征颜色，选用最接近的主题色  
  入口：时间表右上角外观选项 -> 主题色
- 更多的课表块颜色搭配：加入了五组配色方案以配合不同背景及主题  
  入口：时间表右上角外观选项 -> 选择配色方案；另外可以通过点击课程块进行调整
